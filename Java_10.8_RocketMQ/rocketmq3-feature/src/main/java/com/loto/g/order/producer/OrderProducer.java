...
public class OrderProducer {
    public static void main(String[] args) throws MQClientException, RemotingException, InterruptedException, MQBrokerException {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            message = new Message("tp_demo_11", ("hello TD order create - " + i).getBytes());
            producer.send(message, messageQueue);
            message = new Message("tp_demo_11", ("hello TD order pay - " + i).getBytes());
            producer.send(message, messageQueue);
            message = new Message("tp_demo_11", ("hello TD order ship - " + i).getBytes());
            producer.send(message, messageQueue);
        }
